...
static void free_bst_nodes(node_t *node, const enum free_lines free_node_brand)
{
  if (node == NULL) return;

  free_bst_nodes(node->left_child, free_node_brand);
  free_bst_nodes(node->right_child, free_node_brand);
...
{
  free_bst_nodes(bst->root, YES);
  free(bst->root);
  free(bst);
}

static int add_to_node(node_t *node, char *line_models)
{
  char *p;
  size_t n_node_models;

  // Change node string '\0' for ','
  n_node_models = arr_len(node->models);
  for (size_t i = 0; i < n_node_models; ++i) {
    *(node->models[i]-1*sizeof(char)) = ',';
  }

  // line_models starts at the first M in "B'\0'M1,..." change '\0' to ',' and point to it
  line_models -= 1*sizeof(char);
  *line_models = ',';

  // Make space for brand and all models
  p = (char *) realloc(node->brand, (str_len(node->brand)+str_len(line_models)+1)*sizeof(char));
  if (p == NULL) return 1;
...
static node_t *create_node(char *line_brand, char *line_models)
{
  node_t *node;
...
int add_line_to_bst(bst_t *bst, char *line)
{
  ...
  if (curr == NULL) {
    if ((bst->root = create_node(line_brand, line)) == NULL) return 1;
    return 0;
  }
  ...
  while (curr != NULL) {
    ...
    if (comp == 0) {
      return add_to_node(curr, line);
    }
    new_node_p = (comp > 0 ? &curr->left_child : &curr->right_child);
    curr = *new_node_p;
  }

  if ((*new_node_p = create_node(line_brand, line)) == NULL) return 1;

  return 0;
}

static int add_lines_to_bst(bst_t *bst, char **lines, size_t high)
{
  size_t mid = high/2;
  ...
  if (mid != 0)
    if (add_lines_to_bst(bst, lines, mid-1) != 0) return 1;

  if (high != 0)
    if (add_lines_to_bst(bst, &lines[mid+1], high-mid-1) != 0) return 1;
...
bst_t *create_bst(FILE *file)
{
  ...
  // Process file so there is only one brand per line and all models of that brand following it
  if ((n_lines = read_file(file, &lines)) == 0) return NULL;

  // Perform binary search in lines to create balance BST
  if (add_lines_to_bst(bst, lines, n_lines-1) != 0) {
    // bst nodes are pointing to the pointers in lines. So, char * would be free in free_bst()
    free_char_pp(lines);
    free_bst_nodes(bst->root, /* free_lines =*/NO);
    free(bst->root);
    free(bst);
    bst = NULL;
  }

  // Don't free individual char * because a node is pointing to it
  free(lines);
...
static void write_node(const node_t *node, FILE *fp)
{
  char **p;
  char *prev;
  ...
  write_node(node->right_child, fp);
...
{
  write_node(bst->root, file);
}

void print_bst(const bst_t *bst)
{
  write_node(bst->root, stdout);
}

static int node_height(const node_t *node)
{
  int left_height;
  int right_height;
  ...
  left_height = node_height(node->left_child);
  right_height = node_height(node->right_child);
