...
static char **make_null_terminated(arr_str *lines)
{
  char **arr_lines;
  void *temp;
...
static char *remove_last_non_printable(char *s, size_t size)
{
  --size;
  while (s[size] < ' ' || s[size] > '~')
    s[size--] = '\0';
...
static int add_line(arr_str *lines, char *line, const enum try_combine_brands combine)
{
  size_t line_size;
  char *line_copy;
  ...
  if (combine == YES && lines->n_lines > 0) {
    // If this line brand and previous line brand are the same then it must be combine
    char *prev_line;
    char *brand_end;
    ...
    // If the brands are the same, combine models in single char * variable, update lines and return
    if (str_n_cmp(prev_line, line, brand_end-line) == 0) {
      char *new_models;
      char *temp;

      new_models = &line[brand_end-line];
      temp = (char *) realloc(prev_line, str_len(prev_line)+str_len(new_models)+1);
...
static arr_str *init_arr_str(void)
{
  arr_str *lines;
...
size_t read_file(FILE *file, char ***arr_lines)
{
  ...
  while ((n_chars = getline(&buffer, &bufsize, file)) > ((ssize_t) 0)) {
    // Remove last non-printable characters from buffer, make it null terminated and add file line to lines
    if (add_line(lines, remove_last_non_printable(buffer, (size_t) n_chars), /* try_combine =*/ YES) != 0) {
  ...
  *arr_lines = make_null_terminated(lines);
